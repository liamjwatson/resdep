...
# --- exp variables
direction 			: Literal['Y'] = 'Y'# Spin axis up means kick should be in y (perp B field)
...
# set_scraper_lower	: float  = 14.15 	# mm
# set_scraper_inner	: float  = 35.50 	# mm
...
response = input("How long do you want to run the experiment?\n(format: hh:mm, e.g. 02:30 for 2 hours 30 minutes)\n").strip().lower()
intrinsic_res_freq 	: float = f_rev * (tune + 0)									# 0th order, kHz
res_freq		   	: float = f_rev * (tune + harmonic)								# harmoinc order, kHz
sweep_time			: float = int(response[:2]) * 3600 + int(response[-2:]) * 60	# seconds
sweep_range			: float = sweep_time * sweep_rate / 1e3 						# kHz
freq_bounds			: float = sweep_range/2 										# kHz
bounds				: float = freq_bounds / (f_rev*(tune+6))						# %
...
set_sweep_freq 		: float 		= sweep_limits[0] 								# kHz
dwell_time			: float 		= sweep_step_size / sweep_rate 					# s
print(f'Estimated Energy scan range = +- {bounds*100:0.2f}%')

# NOTE: below is the old code for calculating from bounds input, not time 
# # --- calcs
# intrinsic_res_freq 	: float = f_rev * (tune + 0)							# 0th order, kHz
# res_freq		   	: float = f_rev * (tune + harmonic)						# harmoinc order, kHz
# if tune in [v_s, v_s303GeV]:
